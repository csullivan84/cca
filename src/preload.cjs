...
contextBridge.exposeInMainWorld('cca',Object.freeze({
  ...
  onAction:callback=>{const fn=(_,action)=>callback(action);ipcRenderer.on('cca:action',fn);return()=>ipcRenderer.removeListener('cca:action',fn)},
  onNotice:callback=>{const fn=(_,message)=>callback(message);ipcRenderer.on('cca:notice',fn);return()=>ipcRenderer.removeListener('cca:notice',fn)}
}))
