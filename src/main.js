...
if (!dataDir && process.platform === 'win32') {
  ...
  if (fs.existsSync(path.join(root, 'portable.txt')))
...
try {
  ...
  dataDir = path.join(app.getPath('appData'), 'cca-independent')
  fs.mkdirSync(dataDir, { recursive: true })
}
...
function readState() {
  try {
    ...
    return validate(JSON.parse(fs.readFileSync(storePath, 'utf8')))
  } catch (error) {
    if (error.code !== 'ENOENT') {
      startupWarning =
        'Saved settings could not be read. A backup was kept; defaults are active.'
      try {
  ...
  try {
    ...
    if (win && !win.isDestroyed())
      win.webContents.send(
        'cca:notice',
        'Could not save settings. Check free space and folder permissions.'
      )
  }
}
...
function handle(name, fn) {
  ipcMain.handle('cca:' + name, async (event, ...args) => {
...
const languages = Object.fromEntries(
  fs
    .readdirSync(path.join(__dirname, 'views/translations'))
    .filter((f) => /^[a-z]{2}(?:-[A-Za-z]{2,4})?\.json$/.test(f))
    .map((f) => [
      f.slice(0, -5),
      JSON.parse(
        fs.readFileSync(path.join(__dirname, 'views/translations', f), 'utf8')
      )
    ])
)
handle('load', () => ({
...
handle('save', (input) => {
  ...
  next.bounds = state.bounds
  state = next
  win.setAlwaysOnTop(state.settings.alwaysOnTop)
  scheduleSave()
  return true
})
handle('copy', async (value) => {
  await clipboard.writeText(text(value, 200000))
  return true
})
handle('export', async (rows, type) => {
  if (!['html', 'txt', 'json', 'csv'].includes(type))
    throw new Error('Invalid export type')
  const content = report(rows, type)
  const result = await dialog.showSaveDialog(win, {
    title: 'Export contrast report',
    defaultPath: `cca-report.${type}`,
...
handle('openHelp', async (key) => {
  const urls = {
    ...
    upstream: 'https://github.com/ThePacielloGroup/CCAe'
  }
  if (!Object.hasOwn(urls, key)) throw new Error('Unknown help link')
  await shell.openExternal(urls[key])
})
handle('updates', async () => {
  const result = await updates.check(packageInfo.version)
  pendingUpdate = result.available ? result.data : null
  return {
...
handle('downloadUpdate', async () => {
  ...
  const result = await dialog.showSaveDialog(win, {
    title: 'Save verified CCA update',
    defaultPath: `cca-${info.version}-${process.platform}-${process.arch}.zip`,
    filters: [{ name: 'ZIP archive', extensions: ['zip'] }]
  })
  if (result.canceled) return false
  await updates.download(info, result.filePath)
  return true
})
function createWindow() {
  ...
  win = new BrowserWindow({
    ...
    minWidth: 360,
    minHeight: 400,
    title: 'CCA — Colour Contrast Analyser',
    show: false,
  ...
  app
    ...
    .then(() => {
      ...
      protocol.handle('cca', (request) => {
        ...
        if (url.hostname !== 'app' || !Object.hasOwn(allowed, url.pathname))
          return new Response('Not found', { status: 404 })
        return net.fetch(
          pathToFileURL(path.join(__dirname, 'ui', allowed[url.pathname])).href
        )
      })
        ...
        Menu.buildFromTemplate([
          {
            label: 'CCA',
            submenu: [
              { label: 'About CCA', accelerator: 'F1', click: action('about') },
              {
                label: 'Preferences',
          ...
          },
          {
            ...
            submenu: [
              ...
              { role: 'selectAll' },
              { type: 'separator' },
              { label: 'Read contrast result', click: action('read') },
              { label: 'Copy contrast report', click: action('copy') },
              { label: 'Undo colour change', click: action('undo') },
              { label: 'Redo colour change', click: action('redo') }
            ]
          },
          {
            label: 'View',
            submenu: [
              ...
              {
                label: 'Centre window on primary display',
                click: () => {
                  if (win)
                    win.setBounds(recoverBounds(null, screen.getAllDisplays()))
                }
              },
              { label: 'Keyboard shortcuts', click: action('settings') }
            ]
          }
        ])
      )
      ...
      const recover = () => {
        if (win && !win.isDestroyed())
          win.setBounds(recoverBounds(win.getBounds(), screen.getAllDisplays()))
      }
